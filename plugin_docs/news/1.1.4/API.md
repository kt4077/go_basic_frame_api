...
- 无接口变化；管理端文章正文改用宿主统一富文本组件并适配亮色、暗色主题。

### 1.1.3

- 接口文档调整为按插件版本独立归档，不涉及接口字段变化。

### 1.1.2

- 补充各发行版本的独立更新说明，不涉及接口和数据库结构变化。

### 1.1.1

- 优化管理端文章列表的封面、摘要及阅读数量展示，不涉及接口和数据库结构变更。
- 新增分类和文章时自动生成标识，创建后禁止修改稳定标识。

### 1.1.0

- 管理端文章列表新增启用状态筛选；
- 管理端文章保存接口新增 `enable_status` 与 `virtual_view_count`；
- 管理端和用户端响应新增真实、虚拟与展示浏览数量；
- 用户端列表与详情只允许访问已发布且启用的文章。
