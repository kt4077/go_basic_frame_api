...
- 菜单表单的后端接口改为一行一个的可增删编辑器，列表也按行展示；提交时自动去空、去重、规范请求方法大写并拼接为英文逗号分隔字符串，`sys_menu.api_path` 数据结构和后端权限解析逻辑保持不变。
- 顶栏品牌区域与左侧菜单折叠状态联动：折叠后缩小 Logo 并隐藏系统名称；折叠按钮后新增“刷新当前页面”，不刷新浏览器、不重建整体布局。刷新时旧页面保持显示，自动跟踪当前页面初始化接口，待请求完成和新视图渲染后平滑淡出，减少列表、表单闪现；标签栏原有刷新操作同步复用该机制。
- 新增全局请求反馈：GET/HEAD/OPTIONS 查询显示顶部非阻塞“数据加载中”，POST/PUT/PATCH/DELETE 等数据操作显示阻塞式“操作处理中”遮罩；支持并发计数、延迟出现、最短展示时间以及成功/业务失败/网络异常统一收尾，减少闪烁和重复提交。
- 管理端 Axios 超时时间改由 `VITE_ADMIN_API_TIMEOUT_MS` 配置，单位毫秒，默认 30000，允许 1000-300000；非法值回退默认值，特殊接口可单独覆盖，超时统一提示并关闭全局 Loading。环境变量属于 Vite 构建配置，修改后需重启开发服务或重新构建。
