...
从 `v0.0.2` 建立开发分支、分发和安装插件的流程参见 [插件开发、分发、安装与更新指南](./plugin-development-distribution-guide.md)。
...
- 新增 `internal/common/plugin`：
  ...
  - 依赖、版本和循环依赖检查；
  - 插件启动与反向停止生命周期。
- 新增 `internal/plugins/register.go`，集中维护编译进程序的源码插件。
...
当前核心兼容版本为 `0.0.2`。`Manifest.CoreVersion` 留空表示插件自行承担兼容性；填写时必须与当前核心版本精确一致。第一阶段暂不解析语义化版本范围。
...
在 `internal/plugins/register.go` 中加入插件实例：
...
只有写入此清单的插件才会编译进程序。数据库不能启用未编译的代码。
...
执行插件 SQL 后，向 `sys_plugin_migration` 写入成功记录。插件声明了摘要时，数据库记录必须与代码声明一致。
...
普通角色默认不应自动获得新插件权限。由管理员明确授权后，清理权限缓存或重新登录。
