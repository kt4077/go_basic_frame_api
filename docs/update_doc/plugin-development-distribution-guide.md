...
`v0.0.2` 插件使用“独立插件仓库 + 标准 ZIP 发行包 + 安装器 CLI + 编译期注册”模式。
...
接口端和管理端必须使用同一核心标签。正式开发 `v0.0.2` 插件时：
...
在 `v0.0.2` 标签尚未发布时，只能临时从 `release/v0.0.2` 开发；标签发布后必须重新对齐标签并完成全部检查。
...
首次开发从核心 `v0.0.2` 模板创建插件仓库。后续更新从插件仓库自己的 `main` 或上一个插件标签开发，不需要再次合并开发商的历史分支到核心仓库。
...
插件菜单只能引用同一插件包中的 `parent_key`。第一阶段不允许插件直接挂载到某个核心菜单的数字 ID，避免核心菜单调整导致层级错误。
...
`v0.0.2` 不提供自动破坏性卸载命令。卸载需要人工审批、备份和审查，生产环境禁止由 Web 页面自动执行 `DROP TABLE` 或删除源码。
