...
15. 时间展示精确到秒，不显示毫秒。
16. 每个插件必须提供独立的 `docs/API.md` 接口文档，并随仓库、标签和发行 ZIP 一同发布；代码、菜单权限或接口字段变化时必须同步更新文档。
...
`docs/API.md` 是强制交付文件，必须覆盖管理端接口、用户端接口以及第三方回调；没有某类接口时也要明确写明“无”。接口文档不得只链接开发环境中的 Swagger 地址，发行包必须包含可离线阅读的 Markdown 文档。
...
接口文档必须与实际注册路由、Param、Resp 和 `plugin.json.menus[].api_path` 一致。新增、删除或调整接口字段时，应在同一提交中更新 `docs/API.md`；文档未更新视为功能未完成。公开接口也必须记录访问频率、分页上限和内容安全注意事项。自动生成 OpenAPI/Swagger 文档可以作为补充，但不能替代发行包中的 `docs/API.md`。安装器执行 `plugin validate`、`plugin install` 或 `plugin upgrade` 时会检查该文件，缺失或为空的插件包不能通过校验。
...
- [ ] `docs/API.md` 已覆盖全部实际路由，参数、响应、鉴权、权限和错误场景与代码一致。
