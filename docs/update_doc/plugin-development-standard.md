...
新插件开发基线统一为核心 `v0.0.4`，采用编译期注册方式。插件清单中的 `core_version` 必须与实际使用的宿主 `commonplugin.CoreVersion` 一致：
...
10. 数据库枚举值必须从 `1` 开始，并在 Go 枚举文件和前端枚举文件中集中定义，枚举优先查看核心框架是否有，没有的情况在自定义。
...
15. 时间展示精确到秒，不显示毫秒。
16. 每个插件版本必须提供独立的 `docs/api/v{version}.md` 接口文档，并随仓库、标签和发行 ZIP 一同发布；代码、菜单权限或接口字段变化时必须新增对应版本文档。
17. 每个插件版本必须提供独立的 `docs/database/v{version}.md` 数据库说明；有引用必须完整列出，没有引用也必须明确写“无”。缺少或空文档时安装器拒绝校验、安装和升级，Purge 缺少宿主归档文档时拒绝执行。
18. 插件需要富文本编辑时必须复用宿主 `src/components/RichTextEditor.vue`，不得在插件内重复封装 WangEditor 或自行维护明暗主题。
...
`docs/api/v{version}.md` 和 `docs/database/v{version}.md` 都是强制交付文件，版本必须与 `plugin.json.version` 一致。API 文档必须覆盖管理端接口、用户端接口以及第三方回调；数据库说明必须覆盖插件业务表、表内关系、核心表引用、其他插件表引用、外部表反向引用、文件归属和移除影响。没有某类接口或数据库引用时也要明确写明“无”。顶层 `docs/API.md`、`docs/DATABASE.md` 可以作为当前版本入口，但不能替代不可变的版本文档。
...
- `Version` 必须使用语义化版本（SemVer），格式为 `主版本.次版本.修订版本`，例如 `1.2.3`；
- 每一次对外发布都必须提升版本号，禁止覆盖、重新打包或重复发布已经安装的相同版本；
- 不兼容的接口、配置或数据结构调整提升主版本，例如 `1.2.3 → 2.0.0`；
- 向后兼容的新功能提升次版本，例如 `1.2.3 → 1.3.0`；
- 向后兼容的问题修复、UI优化、文档或资源调整提升修订版本，例如 `1.2.3 → 1.2.4`；
- `plugin.json.version`、后端 `Manifest.Version`、发行标签和 ZIP 文件名必须保持一致；
...
- 禁止在升级脚本中静默删除业务数据。
- MySQL DDL 会隐式提交，无法与插件元数据处于同一个物理事务；因此所有 `CREATE TABLE`、新增字段和新增索引迁移必须可安全重试。
- 安装器必须将迁移记录、系统菜单、菜单映射、插件信息和成功安装日志放在同一个事务中提交；任一步骤失败时不得留下孤立菜单或半套元数据。
- 同一插件的安装与升级必须使用数据库级互斥锁串行执行，防止多进程并发重复迁移或重复创建菜单。
...
迁移失败时必须记录状态 `2` 和错误原因，不得把未完成的迁移标记为成功。修改已经发布的 SQL 会造成摘要不一致；已发布迁移不得修改，只能新增更高版本迁移。没有数据库变化的插件版本不需要创建空迁移文件，但插件自身版本仍必须提升。

### 5.5 数据库引用文档

插件每个版本必须提供 `docs/database/v{version}.md`，至少包含：

1. 插件 ID、版本、核心版本和文档更新时间；
2. 所有 `plg_{plugin_id}_*` 业务表、用途和数据敏感级别；
3. 表内关联字段、引用方向、是否为物理外键、删除与更新规则；
4. 对核心表的物理外键或逻辑读写引用，没有时明确写“无”；
5. 对其他插件表的物理外键或逻辑读写引用，并与 `Dependencies` 保持一致，没有时明确写“无”；
6. 其他模块可能反向引用本插件表的情况；
7. 上传文件、缓存、队列、搜索索引等数据库之外的数据归属；
8. `plugin remove` 保留和删除的内容；
9. `plugin purge` 删除的内容、不可恢复风险和备份要求；
10. 是否允许 Purge 的明确结论及限制条件。

插件每次发布还必须提供 `docs/updates/v{version}.md`，文件名版本必须与 `plugin.json.version` 完全一致，至少说明版本类型、主要变化、数据库变化、升级操作、兼容性和回滚方式。仅维护 `CHANGELOG.md` 不能替代独立版本更新说明。

安装器会把插件文档归档到 `server_api/plugin_docs/{plugin_id}/{version}`。Remove 保留归档文档供审查；Purge 必须先确认最新归档版本的 `database/v{version}.md` 存在且非空，再结合数据库真实表和外键执行。旧版本归档的顶层 `DATABASE.md` 仅用于兼容。文档是人工审查依据，不替代运行时外键扫描、插件依赖检查和表前缀校验。

为避免只创建空文档绕过检查，安装器还会校验数据库说明至少包含“插件业务表”“外部数据库引用”“是否引用核心业务表”“是否引用其他插件表”“是否允许 Purge”“是否自动删除文件”“Remove”和“Purge”等明确声明。缺少任一声明都不能安装、升级或彻底清理。
...
1. 插件 ID、插件版本、适配的核心版本和文档更新时间；
2. 接口基础前缀、统一响应结构、认证方式和权限规则；
3. 接口方法、完整路由、接口用途及所属路由组（`Public`、`Auth` 或 `Permission`）；
4. Path、Query、Header、JSON Body 参数，包括类型、是否必填、约束、枚举值和说明；
5. 成功响应字段、分页结构、关联对象、文件相对路径与完整地址字段；
6. 业务失败场景、HTTP 状态码或统一业务错误码；
7. 涉及写操作时说明幂等性、并发行为、事务边界或重复请求结果；
8. 第三方回调说明验签、防重放、重试和幂等策略；
9. 至少提供一组可直接使用的请求和响应示例，示例不得包含真实 Token、密钥或个人数据；
10. 版本变更记录，标注新增、兼容变更和不兼容变更。

接口文档必须与实际注册路由、Param、Resp 和 `plugin.json.menus[].api_path` 一致。每次发布都必须新增 `docs/api/v{version}.md`，即使接口没有变化也要明确写明“无接口变化”；历史版本文件禁止覆盖。公开接口也必须记录访问频率、分页上限和内容安全注意事项。自动生成 OpenAPI/Swagger 文档可以作为补充，但不能替代发行包中的版本接口文档。
...
4. 列表页和新增/修改表单必须拆分：列表页只负责查询、筛选、分页、列表操作和打开组件；表单、弹窗、校验、详情回填及保存逻辑必须放在 `components/*Form.vue` 等独立组件中，通过 Props、Emits 和 `defineExpose` 协作。禁止把完整表单或弹窗直接写在列表页 `index.vue` 中。
5. 页面按钮必须使用现有 `v-perm` 权限指令。
6. 表格、表单、分页、上传、空状态和错误提示复用现有组件及交互规则。
7. 页面不得定义影响其他模块的全局 CSS；样式优先使用 `scoped` 和现有主题变量。
8. 不硬编码后端域名、存储域名、权限值或中文状态映射。
9. 文件上传只向业务接口提交相对路径；展示地址使用后端补全结果。
10. 日期时间统一展示到秒。
11. 新增页面后执行类型检查和生产构建，确认动态路由可以解析。
12. 富文本字段统一使用宿主 `@/components/RichTextEditor.vue`，由宿主维护上传、亮色/暗色主题和编辑器销毁生命周期，插件只绑定业务字段和配置提示文本。
...
3. 使用 `plugin install --apply-database` 安装前后端源码、执行受限迁移并登记插件信息。
   如果此前只安装了源码、数据库中还没有插件记录，必须允许使用同一个发行包重新执行 `plugin install --apply-database` 补齐数据库；不能要求删除插件目录，也不能错误地改用升级命令。
...
安装命令输出 `数据库=false` 代表只完成源码安装，不代表插件已经完成安装。完整安装必须确认业务表、`sys_plugin`、`sys_plugin_migration`、`sys_plugin_menu` 和 `sys_plugin_install_log` 均已正确写入；输出和部署文档必须清楚提示是否执行了数据库阶段。
...
- 停用只修改插件状态并重启服务，不删除业务数据；插件必须先停用才能移除。
- `plugin remove <plugin_id>` 是可恢复移除：事务删除角色菜单授权、系统菜单、菜单映射和插件记录，然后移除前后端源码并重新生成注册文件；业务表、迁移记录、安装日志和上传文件继续保留。
- 插件信息和插件菜单必须物理删除，避免软删除记录继续占用 `plugin_id`、菜单 ID 等唯一索引；业务数据仍由保留的插件业务表承载。
- 移除前必须检查没有已启用插件依赖当前插件。数据库元数据先删除，即使源码阶段意外中断，插件也不会重新运行；重复执行命令可以继续清理遗留源码。
- 移除后使用原发行包重新安装时，安装器复用已登记的迁移，不重复执行业务表迁移，并重新生成菜单和映射。
- `plugin purge <plugin_id> --confirm <plugin_id>` 是不可恢复的彻底清理：先执行可恢复移除，再删除 `plg_{plugin_id}_*` 业务表；所有业务表删除成功后，才事务删除迁移历史和安装日志。
- Purge 执行前检查是否存在插件前缀以外的外键引用；存在时拒绝清理。插件内部外键只在当前数据库连接中临时关闭检查，并确保恢复。
- MySQL DDL 不能事务回滚，因此 Purge 使用“先使插件不可运行、DDL 可重复执行、最后删除历史记录”的可恢复状态机；中断后再次执行相同命令继续完成。
- 上传文件默认不自动删除，因为当前上传记录没有强制插件归属字段；文件清理由管理员根据业务归属单独审查。
- 删除数据、文件和表必须由管理员明确执行并提前备份；回滚代码前确认数据库结构和插件版本与旧代码兼容。
...
- [ ] `docs/api/v{version}.md` 已覆盖全部实际路由，参数、响应、鉴权、权限和错误场景与代码一致。
- [ ] 接口变更已同步更新 API 文档中的版本变更记录和示例。
...
- [ ] SQL 不会删除或覆盖现有业务数据。
- [ ] 迁移 SQL 可安全重试，元数据在同一事务提交，并发安装受插件级数据库锁保护。
- [ ] 迁移版本和 SHA256 摘要已写入迁移记录。
- [ ] 菜单业务键唯一、父级存在，清单中没有固定菜单 ID。
- [ ] `docs/database/v{version}.md` 已列出全部业务表、内外部引用、文件归属和 Remove/Purge 影响；没有引用的项目已明确写“无”。

### 管理端

- [ ] 页面路径和文件目录满足插件页面解析约定。
- [ ] 使用 TypeScript、ES6 和箭头函数。
- [ ] 新增/修改表单已拆成独立组件，列表页没有内嵌完整表单、详情回填或保存逻辑。
- [ ] 请求通过统一封装，未硬编码接口域名。
- [ ] 操作按钮使用 `v-perm`。
